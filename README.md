...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
    ...
    `actions`: actions are javascript objects that have a 'type' property indicating the type of action that will occur and sometimes a 'payload' property that indicates what will be changed in state. 

    `reducers`: a reducer is a function that takes two arguments (state/initialState and action). The function then typically returns a switch statement containing your actions with an updated state.

    `store`: the store contains the state for the app. It is known as a 'single source of truth' because with using Redux and store, every state piece for any of the components will be in the one store making our state immutable.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

    Application state is global while component state is local. Application state is good to use for big app state because any component anywhere in the app could access it. Its good if multiple components need to access that state. Component state is good for smaller app states because they live in that single component. So, it can only be updated within that component.


- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

    `redux-thunk` is a node package that allows our reducer flow to become asynchronous so that we can make API calls in our action creators.


- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

    I quite enjoyed Redux, in was a little confusing as first but once I understood it, I can see myself needing it once I start making bigger applications. And once I understood it, it seemed quite easy to read, having state plus all of the actions(events) that are associated with state all together seems like a good idea. The reducer pattern was a great easier intro into learning about actions though.
